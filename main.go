...
func main() {
	...
	programVersionFlag := getopt.BoolLong("version", 'v', "Displays the version of tfselect")
	latestVersionFlag := getopt.BoolLong("latest", 'u', "Switch to the latest terraform version")
	//preVersionFlag := getopt.BoolLong("pre", 'p', "Latest pre-release implicit version. Ex: tfselect --pre 0.13 downloads 0.13.0-rc1 (latest)\"")
	helpFlag := getopt.BoolLong("help", 'h', "Displays help message")
	initFlag := getopt.BoolLong("save", 's', "Generate .tfselect.toml in current directory with current version terraform")
	...
	envPath := os.Getenv("TFSELECT_PATH")
	if envPath != "" {
		fmt.Println("TFSELECT_PATH detected")
		path = envPath
	}

	envVersion := os.Getenv("TFSELECT_VERSION")
	if envVersion != "" {
		fmt.Println("TFSELECT_VERSION  detected")
